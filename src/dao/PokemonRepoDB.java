...
public class PokemonRepoDB implements IPokemonRepo
{
    ...
    public void addPokemon(Pokemon pokemon)
    {
        try
        {
            ...
            PreparedStatement evStatement = cm.getConnection().prepareStatement("INSERT INTO evs_table (hp, atk, def, spa, spd, spe) VALUES (?, ?, ?, ?, ?, ?)", Statement.RETURN_GENERATED_KEYS);
            ...
            PreparedStatement pokeStatement = cm.getConnection().prepareStatement("INSERT INTO pokemon_table (pokemon_name, nickname, lvl, ability, item, evid, nature, gender, shiny, ot) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?);", Statement.RETURN_GENERATED_KEYS);
            ...
            PreparedStatement moveStatement = cm.getConnection().prepareStatement("INSERT INTO moves_table (move_name) VALUES (?)", Statement.RETURN_GENERATED_KEYS);
            ...
            PreparedStatement pm = cm.getConnection().prepareStatement("INSERT INTO pokemon_moves (p_id, move_id) VALUES (?, ?)");
            pm.setInt(1, p_id);
            for(int i = 0; i < 4; i++)
            {
                if(move_ids[i] != 0)
    ...
    public Pokemon getPokemon(int id)
    {
        try
        {
            PreparedStatement ps = cm.getConnection().prepareStatement
